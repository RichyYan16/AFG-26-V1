/**
 * Type definitions for the hybrid embedding + Openrouter diagnostic model
 * 
 * Boilerplate code generated by Claude Haiku 4.5 based on the following prompt:
 * "Give me boilerplate code that allows me to implement types and interfaces for a hybrid embedding + OpenRouter diagnostic model in an educational app. Include types for stuck types, emotions, cognitive distortions, diagnostic questions and answers, diagnostic context (e.g., time stuck, energy level), diagnosis results (including confidence scores and ranked types), intervention plans, session tracking, student profiles, and trend insights. Ensure the types are comprehensive and structured to support the full diagnostic and intervention workflow."
 * 
 * All logic using these types was implemented by the authors
 */
