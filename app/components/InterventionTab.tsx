  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4">
        ...
        <p className="text-emerald-200">Generating personalized strategies based on your assessment...</p>
      </div>
      ...
      {loadingInterventions ? (
        ...
        <div className="space-y-4">
                ...
                <div key={index} className="p-4 rounded-lg border border-emerald-800 bg-emerald-950/60">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <p className="text-emerald-100 mb-2">{plan.action}</p>
                      {plan.resources && plan.resources.length > 0 && (
                        <div className="mt-3">
                          <h4 className="text-sm font-semibold text-lime-200 mb-2">Helpful Resources:</h4>
                          <div className="flex flex-wrap gap-2">
                            {plan.resources.map((resource, resourceIndex) => (
                              <span key={resourceIndex} className="px-2 py-1 text-xs bg-emerald-800 text-emerald-200 rounded-full border border-emerald-700">
                                {resource}
                              </span>
                            ))}
                          </div>
          ...
          <div className="text-center space-y-3">
            ...
            >
              {saving ? "Saving..." : "Save Assessment & Plans"}
            </button>
          </div>
        ...
        <div className="text-center py-12">
          <p className="text-emerald-200">No intervention plans available. Please complete the assessment first.</p>
          <button
            type="button"
            onClick={onNavigateToResult}
